...
template<typename Dtype>
void DataTransformer<Dtype>::TransformImgAndSeg_video(
    const std::vector<cv::Mat>& cv_img_seg,
    Blob<Dtype>* transformed_data_blob,
    Blob<Dtype>* transformed_label_blob,
    const int ignore_label,const int bg_channels_offset) {
  CHECK(cv_img_seg.size() == 3) << "Input must contain image, seg and bg.";
  ...
  CHECK_EQ(seg_channels, 1);
  CHECK_EQ(img_channels*2, data_channels);
  CHECK_EQ(img_height, seg_height);
  CHECK_EQ(img_width, seg_width);
  ...
  cv::Mat cv_bg_img = cv_img_seg[2];
  cv_bg_img.convertTo(cv_bg_img,CV_64F);
  ...
  for (int h = 0; h < data_height; ++h) {
    ...
    for (int w = 0; w < data_width; ++w) {
      ...
      for (int c = 0; c < img_channels; ++c) {
        ...
        //for background image
        bg_index = ((c + bg_channels_offset) * data_height + h) * data_width + w;
        Dtype bg_pixel = static_cast<Dtype>(bg_ptr[data_index]);
        ...
        if (has_mean_file) {
          ...
          if (has_mean_values) {
            transformed_data[top_index] =
              (pixel - mean_values_[c]) * scale;
            CHECK_LE(bg_index,6*transformed_data_blob->height()*transformed_data_blob->width()-1);
            //transformed_data[bg_index]; 
            transformed_data[bg_index] =
              (bg_pixel - mean_values_[c]);
          } else {
            transformed_data[top_index] = pixel * scale;

            transformed_data[bg_index] = bg_pixel;
          }
        }
      }
